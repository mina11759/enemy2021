...
def preprocess_dataset(dataset_name, min_sent_num=1, max_sent_num=50): # data extraction
    ...
    for item in tqdm(data):
        if item['owner'] == 'nobody@mozilla.org':
            ...
            owner = item['owner']
            clean_data = clean_sent_list(item) # tokenize sentence
            sentence_num = len(clean_data)

            if sentence_num == 1:
                continue

            if sentence_num > max_sent_num or sentence_num < min_sent_num: # test
                continue
